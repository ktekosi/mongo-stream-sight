...
describe('Server Integration Tests', () => {
    ...
    beforeAll(async() => {
        ...
        serverProcess = Bun.spawn(['bun', 'src/index.ts', '-m', 'mongodb://root:password@mongo1,mongo2,mongo3/admin?replicaSet=rs0']);

        await waitPort({ port: LISTEN_PORT, protocol: 'http', interval: 1000, timeout: 2000 });
    });
